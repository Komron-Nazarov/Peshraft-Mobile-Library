...
func (s *BookService) GetAll(page, pageSize int, currentUserID uint) ([]models.Book, int, error) {
	books, total, err := s.bookRepo.FindAll(page, pageSize, currentUserID)
	if err != nil {
		return nil, 0, fmt.Errorf("failed to fetch books: %w", err)
	}
...
func (s *BookService) GetByID(id uint, currentUserID uint) (*models.Book, error) {
	book, err := s.bookRepo.FindByID(id, currentUserID)
	if err != nil {
		return nil, fmt.Errorf("book not found: %w", err)
	}
	return book, nil
...
func (s *BookService) Search(params models.BookSearchParams, currentUserID uint) ([]models.Book, int, error) {
	books, total, err := s.bookRepo.Search(params, currentUserID)
	if err != nil {
		return nil, 0, fmt.Errorf("search failed: %w", err)
	}
	return books, total, nil
